...
class AppInput(BaseAppInput):
    ...
    instruction: str = Field(description="Text description of the object to add")
    mask: Optional[File] = Field(default=None, description="Binary mask defining placement region (white=place, black=keep)")
...
class AppOutput(BaseAppOutput):
    image: File = Field(description="Image with added object")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Add Object ready")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        if input_data.mask is not None:
            payload["mask"] = input_data.mask.uri
        ...
        logger.info("Requesting object addition")
        result = await bria_helper.call_endpoint(self.client, "add_object_by_text", payload)
