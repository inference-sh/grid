...
class AppInput(BaseAppInput):
    ...
    object_name: str = Field(description="Text description of the object to erase")
    seed: Optional[int] = Field(default=None, description="Seed for reproducible results (0-2147483647)")


class AppOutput(BaseAppOutput):
    image: File = Field(description="Image with described object erased")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Erase by Text ready")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        logger.info("Requesting text-based erasure")
        result = await bria_helper.call_endpoint(self.client, "erase_by_text", payload)
