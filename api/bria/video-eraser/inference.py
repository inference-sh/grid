...
class AppInput(BaseAppInput):
    ...
    mask_url: str = Field(description="URL of the mask video defining regions to erase (white=erase, black=keep)")
...
class AppOutput(BaseAppOutput):
    video: File = Field(description="Video with masked objects erased and inpainted")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client(timeout=300)
        logger.info("Bria Video Eraser ready")

    async def run(self, input_data: AppInput) -> AppOutput:
        payload = {
            "video": input_data.video_url,
            "mask": input_data.mask_url,
        }
        ...
        logger.info("Requesting video object erasure")
        result = await bria_helper.call_endpoint(
            self.client, "erase", payload,
        ...
        logger.info(f"Downloaded erased video to {path}")
