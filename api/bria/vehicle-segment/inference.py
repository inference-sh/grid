...
class AppInput(BaseAppInput):
    image: File = Field(description="Vehicle image to segment (JPEG, PNG, WEBP, max 12MB)")
    content_moderation: Optional[bool] = Field(default=None, description="Apply content moderation to input and output images")


class AppOutput(BaseAppOutput):
    image: File = Field(description="Segmented vehicle image with labeled parts (windows, wheels, body, etc.)")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Vehicle Segment ready")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        logger.info("Requesting vehicle segmentation")
        result = await bria_helper.call_endpoint(self.client, "vehicle/segment", payload, base_url=PRODUCT_BASE)
        ...
        logger.info(f"Downloaded segmentation to {path}")
