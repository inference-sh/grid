...
class AppOutput(BaseAppOutput):
    image: File = Field(description="Image with foreground erased, background preserved")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Erase Foreground ready")

    async def run(self, input_data: AppInput) -> AppOutput:
        payload = {"image": input_data.image.uri}

        logger.info("Requesting foreground erasure")
        result = await bria_helper.call_endpoint(self.client, "erase_foreground", payload)
