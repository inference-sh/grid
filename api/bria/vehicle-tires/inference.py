...
class AppInput(BaseAppInput):
    image: File = Field(description="Vehicle image to enhance tires (JPEG, PNG, WEBP, max 12MB)")
    content_moderation: Optional[bool] = Field(default=None, description="Apply content moderation to input and output images")


class AppOutput(BaseAppOutput):
    image: File = Field(description="Vehicle image with enhanced tire appearance")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Vehicle Tires ready")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        logger.info("Requesting tire refinement")
        result = await bria_helper.call_endpoint(self.client, "vehicle/refine_tires", payload, base_url=PRODUCT_BASE)
        ...
        logger.info(f"Downloaded refined tires image to {path}")
