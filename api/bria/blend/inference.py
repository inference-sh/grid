...
class AppInput(BaseAppInput):
    image: File = Field(description="Source image to blend into (JPEG, PNG, WEBP)")
    instruction: str = Field(description="Natural language description of the blend edit")
    seed: Optional[int] = Field(default=None, description="Seed for reproducible results")


class AppOutput(BaseAppOutput):
    image: File = Field(description="Blended image")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Blend ready")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        logger.info("Requesting image blend")
        result = await bria_helper.call_endpoint(self.client, "blend", payload)

        image_url = result["result"]["image_url"]
        path = await bria_helper.download_image(self.client, image_url)
        logger.info(f"Downloaded blended image to {path}")
