...
class AppOutput(BaseAppOutput):
    image: File = Field(description="Image with blurred background")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Blur Background ready")

    async def run(self, input_data: AppInput) -> AppOutput:
        payload = {"image": input_data.image.uri}

        logger.info("Requesting background blur")
        result = await bria_helper.call_endpoint(self.client, "blur_background", payload)

        image_url = result["result"]["image_url"]
        path = await bria_helper.download_image(self.client, image_url)
        logger.info(f"Downloaded blurred image to {path}")
