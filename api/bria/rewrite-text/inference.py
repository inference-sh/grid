...
class AppInput(BaseAppInput):
    image: File = Field(description="Source image containing text (JPEG, PNG, WEBP)")
    new_text: str = Field(description="The new text to write in the image, replacing existing text")
    seed: Optional[int] = Field(default=None, description="Seed for reproducible results (0-2147483647)")


class AppOutput(BaseAppOutput):
    image: File = Field(description="Image with rewritten text")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Rewrite Text ready")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        logger.info("Requesting text rewrite")
        result = await bria_helper.call_endpoint(self.client, "rewrite_text", payload)
