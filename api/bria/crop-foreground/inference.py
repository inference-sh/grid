...
class AppOutput(BaseAppOutput):
    image: File = Field(description="Cropped foreground object")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Crop Foreground ready")

    async def run(self, input_data: AppInput) -> AppOutput:
        payload = {"image": input_data.image.uri}

        logger.info("Requesting foreground crop")
        result = await bria_helper.call_endpoint(self.client, "crop_foreground", payload)

        image_url = result["result"]["image_url"]
        path = await bria_helper.download_image(self.client, image_url)
        logger.info(f"Downloaded cropped image to {path}")
