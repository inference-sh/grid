...
class AppInput(BaseAppInput):
    ...
    mask: File = Field(description="Binary mask defining fill region (white=fill, black=keep)")
    prompt: str = Field(description="Text description of content to generate in the masked area")
...
class AppOutput(BaseAppOutput):
    image: File = Field(description="Image with masked region filled by generated content")


class App(BaseApp):
    async def setup(self, config):
        self.client = bria_helper.get_client()
        logger.info("Bria Gen Fill ready")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        logger.info("Requesting generative fill")
        result = await bria_helper.call_endpoint(self.client, "gen_fill", payload)

        image_url = result["result"]["image_url"]
        path = await bria_helper.download_image(self.client, image_url)
        logger.info(f"Downloaded gen-fill image to {path}")
