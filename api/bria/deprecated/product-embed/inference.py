...
class AppInput(BaseAppInput):
    image: File = Field(description="Product cutout — use product-cutout first to extract from background (JPEG, PNG, WEBP, max 12MB)")
    scene: File = Field(description="Scene image to embed the product into — the background/environment")
    placement: Optional[dict] = Field(default=None, description="Precise product placement in the scene: {'x': int, 'y': int, 'width': int, 'height': int} in pixels")
