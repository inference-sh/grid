"""
Qwen-Image-2.0 - Alibaba Cloud Image Generation

Generate and edit images using Qwen-Image-2.0 model via DashScope API.
Supports text-to-image generation and multi-image editing/fusion.
"""
...
class AppInput(BaseAppInput):
    """Input schema for Qwen-Image-2.0."""

    prompt: str = Field(
        description="Text prompt describing what to generate or edit. For editing, reference images by position (e.g., 'Image 1', 'Image 2')."
    )
    ...
    prompt_extend: bool = Field(
        default=True,
        description="Enable prompt rewriting for more diverse content. Disable for precise control."
    )
...
class AppOutput(BaseAppOutput):
    """Output schema for Qwen-Image-2.0."""

    images: List[File] = Field(description="Generated images in PNG format.")


class App(BaseApp):
    """Qwen-Image-2.0 image generation/editing application."""

    async def setup(self, metadata):
        """Initialize the application."""
        logging.basicConfig(level=logging.INFO)
        self.logger = logging.getLogger(__name__)
        self.model = "qwen-image-2.0"
        self.logger.info(f"Qwen-Image-2.0 initialized")

    async def run(self, input_data: AppInput, metadata) -> AppOutput:
        """Generate or edit images using Qwen-Image-2.0."""
        try:
            # Determine mode
