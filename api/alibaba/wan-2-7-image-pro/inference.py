"""
Wan 2.7 Image Pro - Alibaba Cloud Professional Image Generation

Generate and edit images using Wan 2.7 Image Pro model via DashScope API.
Supports text-to-image, image editing, interactive editing, and image set generation.
Pro edition supports 4K high-definition output for text-to-image.
"""
...
class AppInput(BaseAppInput):
    """Input schema for Wan 2.7 Image Pro."""
    ...
    size: str = Field(
        default="2K",
        description="Output resolution: '1K' (1024x1024), '2K' (2048x2048, default), '4K' (4096x4096, text-to-image only). Or specify pixel dimensions like '1024*768'."
    )
...
class AppOutput(BaseAppOutput):
    """Output schema for Wan 2.7 Image Pro."""

    images: List[File] = Field(description="Generated images in PNG format.")


class App(BaseApp):
    """Wan 2.7 Image Pro generation/editing application."""

    async def setup(self, metadata):
        """Initialize the application."""
        logging.basicConfig(level=logging.INFO)
        self.logger = logging.getLogger(__name__)
        self.model = "wan2.7-image-pro"
        self.logger.info("Wan 2.7 Image Pro initialized")

    async def run(self, input_data: AppInput, metadata) -> AppOutput:
        """Generate or edit images using Wan 2.7 Image Pro."""
        try:
                ...
                output_meta=OutputMeta(
                    outputs=[
                        ImageMeta(
                            ...
                            height=height,
                            count=len(images),
                            extra={"mode": mode, "model": "wan2.7-image-pro"},
                        )
                    ]
                ),
            )
