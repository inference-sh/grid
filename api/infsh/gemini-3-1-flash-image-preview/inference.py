"""
Gemini 3.1 Flash Image Preview with fal.ai fallback.

Uses Vertex AI with automatic fallback to fal.ai (nano-banana-2) on rate limits.
"""
...
class AppInput(BaseAppInput):
    ...
    images: Optional[List[File]] = Field(
        None,
        description="Optional input images for editing (up to 14). Supported formats: JPEG, PNG, WebP"
    )
    ...
    aspect_ratio: AspectRatioAutoEnum = Field(
        ...
        description="Aspect ratio. Use 'auto' to match input image."
    )
...
class AppOutput(BaseAppOutput):
    ...
    description: str = Field(default="", description="Text description from model")
...
class App(BaseApp):
    async def setup(self):
        ...
        self.fal_text_model = "fal-ai/nano-banana-2"
        self.fal_edit_model = "fal-ai/nano-banana-2/edit"
        self.client = create_vertex_client()
        self.logger.info("Gemini 3.1 Flash Image Preview (with fal fallback) initialized")
    ...
    async def _generate_fal(self, input_data: AppInput, aspect_ratio_value: str) -> list:
        ...
        for i in range(input_data.num_images):
            ...
            request = {
                ...
                "enable_web_search": input_data.enable_google_search,
            }
