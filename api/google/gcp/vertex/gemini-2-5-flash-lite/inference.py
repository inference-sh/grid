...
class App(OpenAIChatMixin, BaseApp):
    ...
    async def setup(self):
        ...
        self.logger.info("Gemini 2.5 Flash-Lite (Vertex AI) ready")

    async def run(self, input_data: AppInput) -> AsyncGenerator[Union[LLMDelta, AppOutput], None]:
        last_output = None
        async for output, delta in stream_completion(self.client, input_data, DEFAULT_MODEL, max_tokens=65535, with_deltas=True):
            if delta:
