"""
Kling Video v2.5 Turbo Image-to-Video

Generate videos from images using Kling Video v2.5 Turbo model via fal.ai.
Supports pro and standard tiers.
"""
...
class TierEnum(str, Enum):
    """Quality tier — pro is higher quality, standard is faster and cheaper."""
    pro = "pro"
    standard = "standard"
...
ENDPOINT_BY_TIER = {
    TierEnum.pro: "fal-ai/kling-video/v2.5-turbo/pro/image-to-video",
    TierEnum.standard: "fal-ai/kling-video/v2.5-turbo/standard/image-to-video",
}
...
class AppInput(BaseAppInput):
    """Input schema for Kling Video v2.5 Turbo Image-to-Video."""
    ...
    tail_image: Optional[File] = Field(
        default=None,
        description="Optional tail image for end-frame guidance.",
    )
    ...
    tier: TierEnum = Field(
        ...
        description="Quality tier. Pro is higher quality, standard is faster and cheaper.",
    )


class AppOutput(BaseAppOutput):
    """Output schema for Kling Video v2.5 Turbo Image-to-Video."""
...
class App(BaseApp):
    """Kling Video v2.5 Turbo Image-to-Video application."""

    async def setup(self):
        self.logger = logging.getLogger(__name__)
        self.logger.info("Kling Video v2.5 Turbo I2V initialized")
    ...
    def _build_request(self, input_data: AppInput) -> dict:
        ...
        if input_data.tail_image is not None:
            request["tail_image_url"] = input_data.tail_image.uri
        return request
