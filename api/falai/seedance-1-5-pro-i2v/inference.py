...
Generate videos with audio from images using ByteDance's Seedance 1.5 Pro model.
Supports start and end frame control for seamless video generation.
"""
...
class AppInput(BaseAppInput):
    """Input schema for Seedance 1.5 Pro Image-to-Video."""
    
    prompt: str = Field(
        description="The text prompt used to generate the video. Describe the motion and action.",
        examples=["A man is crying and he says \"I shouldn't have done it. I regret everything\""]
    )
    image: File = Field(
        description="The starting image for video generation. Supported formats: JPEG, PNG, WebP."
    )
    end_image: Optional[File] = Field(
        default=None,
        description="Optional ending image for the video. Creates a transition between start and end frames."
    )
...
class AppOutput(BaseAppOutput):
    """Output schema for Seedance 1.5 Pro Image-to-Video."""
...
class App(BaseApp):
    """Seedance 1.5 Pro Image-to-Video application."""
    
    async def setup(self, metadata):
        """Initialize the application."""
        self.logger = logging.getLogger(__name__)
        
        # fal.ai model endpoint for image-to-video
        self.model_id = "fal-ai/bytedance/seedance/v1.5/pro/image-to-video"
        
        self.logger.info(f"Seedance 1.5 Pro I2V initialized with model: {self.model_id}")
    ...
    def _build_request(self, input_data: AppInput) -> dict:
        ...
        # Optional end image for start/end frame control
