...
Generate videos with synchronized audio from reference images, videos, and/or
audio using ByteDance's Seedance 2.0 model. Refer to references in the prompt
as @Image1, @Video1, @Audio1, etc. Supports quality and fast modes.
"""
...
ENDPOINT_BY_MODE = {
    ModeEnum.quality: "bytedance/seedance-2.0/reference-to-video",
    ModeEnum.fast: "bytedance/seedance-2.0/fast/reference-to-video",
}


class AppInput(BaseAppInput):
    """Input schema for Seedance 2.0 Reference-to-Video."""

    prompt: str = Field(
        description="The text prompt used to generate the video. Reference inputs as @Image1, @Video1, @Audio1, etc.",
        examples=[
            "An octopus finds a football in the ocean and excitedly calls its octopus friends to come and play."
        ],
    )
    images: Optional[List[File]] = Field(
        default=None,
        description="Reference images (@Image1, @Image2, ...). JPEG/PNG/WebP, max 30 MB each, up to 9 images. Total files across all modalities must not exceed 12.",
    )
    videos: Optional[List[File]] = Field(
        default=None,
        description="Reference videos (@Video1, @Video2, ...). MP4/MOV, up to 3 videos, combined 2-15 seconds, total under 50 MB. Each video ~480p to ~720p resolution.",
    )
    audios: Optional[List[File]] = Field(
        default=None,
        description="Reference audio (@Audio1, @Audio2, ...). MP3/WAV, up to 3 files, combined duration up to 15 seconds, max 15 MB per file. Requires at least one reference image or video.",
    )
    mode: ModeEnum = Field(
...
class AppOutput(BaseAppOutput):
    """Output schema for Seedance 2.0 Reference-to-Video."""
...
class App(BaseApp):
    ...
    @staticmethod
    def _uris(files: Optional[List[File]]) -> List[str]:
        if not files:
            return []
        return [f.uri for f in files if f is not None]
    ...
    def _build_request(self, input_data: AppInput) -> dict:
        ...
        image_urls = self._uris(input_data.images)
        video_urls = self._uris(input_data.videos)
        audio_urls = self._uris(input_data.audios)
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        try:
            ...
            for label, files in (
                ("image", input_data.images),
                ("video", input_data.videos),
                ("audio", input_data.audios),
            ):
                if not files:
                    continue
                for idx, f in enumerate(files):
                    if f is None:
                        continue
                    if not f.exists():
                        raise RuntimeError(f"Reference {label} {idx + 1} does not exist at path: {f.path}")

            # Enforce: audio reference requires at least one image or video reference
            has_audio = bool(input_data.audios)
            has_image_or_video = bool(input_data.images) or bool(input_data.videos)
            if has_audio and not has_image_or_video:
                raise RuntimeError("Reference audio requires at least one reference image or video.")
            ...
            self.logger.info(
                f"Settings: {input_data.resolution.value}, {input_data.aspect_ratio.value}, {input_data.duration.value}s, "
                f"images={len(input_data.images or [])}, videos={len(input_data.videos or [])}, audios={len(input_data.audios or [])}, "
                f"endpoint={model_id}"
            )
