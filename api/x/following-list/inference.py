...
class AppInput(BaseAppInput):
    """Input schema for listing accounts a user follows."""
    username: Optional[str] = Field(None, description="Username to get following for (without @)")
    user_id: Optional[str] = Field(None, description="User ID to get following for")
...
class AppOutput(BaseAppOutput):
    """Output schema for following list."""
    user_id: str = Field(description="The user ID whose following was listed")
    users: List[UserInfo] = Field(description="Followed account profiles")
...
class App(BaseApp):
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        try:
            ...
            for page in self.client.users.get_following(
                id=user_id,
