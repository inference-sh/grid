...
class AppInput(BaseAppInput):
    """Input schema for user mentions."""
    max_results: int = Field(default=20, ge=5, le=100, description="Maximum number of results (5-100)")


class AppOutput(BaseAppOutput):
    """Output schema for user mentions."""
    posts: List[PostInfo] = Field(description="List of mentions")
...
class App(BaseApp):
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        try:
            ...
            print(f"Fetching mentions for user {user_id}")

            posts = []
            users_map = {}

            for page in self.client.users.get_mentions(
                id=user_id,
            ...
            print(f"Found {len(posts)} mentions")
