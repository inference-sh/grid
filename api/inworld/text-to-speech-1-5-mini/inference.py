...
Ultra-low-latency TTS (~120ms P50) with 15 languages.
...
class AppInput(BaseAppInput):
    """Input schema for Inworld TTS 1.5 Mini."""
...
class AppOutput(BaseAppOutput):
    """Output schema for Inworld TTS 1.5 Mini."""
    audio: File = Field(description="Generated speech audio file")


class App(BaseApp):
    """Inworld TTS 1.5 Mini app implementation."""

    async def setup(self):
        self.logger = logging.getLogger(__name__)
        get_api_key()
        self.logger.info("Inworld TTS 1.5 Mini app initialized")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        return AppOutput(
            ...
            output_meta=OutputMeta(
                ...
                outputs=[AudioMeta(
                    seconds=duration,
                    extra={"characters": len(input_data.text), "model": "inworld-tts-1.5-mini"},
                )],
            ),
        )
