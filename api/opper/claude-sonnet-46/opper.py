...
def _convert_files_to_documents(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Convert OpenAI file content blocks to Anthropic document format and fix assistant multipart."""
    for msg in messages:
        content = msg.get("content")
        if not isinstance(content, list):
            continue

        # Convert file blocks to document blocks
        ...
        # Simplify assistant/tool messages that only have a text block to plain string
        # Anthropic rejects multipart content arrays on assistant messages with tool_calls
