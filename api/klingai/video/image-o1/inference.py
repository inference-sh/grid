...
Text-to-image and image-to-image with element control.
Supports 1K/2K resolution and intelligent aspect ratio.
"""
...
class AppInput(BaseAppInput):
    """Kolors Image-O1 - text-to-image and image-to-image with element control."""

    prompt: str = Field(description="Text description. Max 2500 chars.")
    image: Optional[File] = Field(default=None, description="Reference image for image-to-image or element control.")
    ...
    resolution: ResolutionEnum = Field(default=ResolutionEnum.k1, description="Output resolution (1K or 2K).")
...
class App(BaseApp):
    async def setup(self, metadata):
        ...
        self.logger.info("Kling Image O1 initialized")
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        output_meta = OutputMeta(outputs=[ImageMeta(count=input_data.n, extra={"model": "kling-image-o1", "mode": mode, "resolution": input_data.resolution.value})])
