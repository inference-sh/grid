"""
Kling Image V2 (Kolors V2.0) - Multi-Image, Restyle & 2K

Text-to-image, multi-image-to-image, and restyle capabilities.
Only model with 2K resolution for text-to-image and restyle output matching input resolution.
"""
...
class AppInput(BaseAppInput):
    """Kolors V2.0 - text-to-image, multi-image reference, and restyle.

    - Text-to-image: prompt only (supports 2K)
    - Restyle: prompt + image (output matches input resolution)
    - Multi-image: prompt + image reference
    """

    prompt: str = Field(description="Text description. Max 2500 chars.")
    image: Optional[File] = Field(default=None, description="Reference image for restyle or multi-image generation. Restyle output matches input resolution.")
    ...
    resolution: ResolutionEnum = Field(default=ResolutionEnum.k1, description="Output resolution. 2K supported for text-to-image.")
...
class App(BaseApp):
    async def setup(self, metadata):
        ...
        self.logger.info("Kling Image V2 initialized")

    async def on_cancel(self):
        return True

    async def run(self, input_data: AppInput) -> AppOutput:
        mode = "restyle" if input_data.image else "text-to-image"
        ...
        output_meta = OutputMeta(outputs=[ImageMeta(count=input_data.n, extra={"model": "kling-v2", "mode": mode, "resolution": input_data.resolution.value})])
