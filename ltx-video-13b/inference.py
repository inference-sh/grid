...
class App(BaseApp):
    async def setup(self):
        ...
        # Load config file
        config_file = "configs/ltxv-13b-0.9.7-dev.yaml"  # Default config file for 13B model
        ...
        # Create multi-scale pipeline if configured
        if self.config.get("pipeline_type") == "multi-scale" and self.spatial_upscaler_path:
            latent_upsampler = LatentUpsampler.from_pretrained(self.spatial_upscaler_path)
    ...
    async def run(self, input_data: AppInput) -> AppOutput:
        ...
        images = self.pipeline(
            ...
            **sample,
            ...
            first_pass=self.config.get("first_pass", {}),
            second_pass=self.config.get("second_pass", {}),
        ...
        if not os.path.exists(output_filename):
            raise FileNotFoundError(f"Failed to generate video file at {output_filename}")
