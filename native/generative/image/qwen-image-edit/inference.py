...
class App(BaseApp):
    ...
    async def run(self, input_data: AppInput, metadata) -> AppOutput:
        ...
        # Unload previously loaded LoRA adapters that are no longer requested or whose config changed
                ...
                for created_name in previous_created:
                    ...
                    if hasattr(self.pipeline.transformer, 'delete_adapters'):
                        try:
                            ...
                            print(f"⚠️ Could not delete adapter '{created_name}' from transformer: {e}")
                    
                    # Try deleting from pipeline if transformer failed
                    if not deleted and hasattr(self.pipeline, 'delete_adapters'):
                        try:
                            ...
                            print(f"⚠️ Could not delete adapter '{created_name}' from pipeline: {e}")
                    else:
        ...
        for lora in loras:
            ...
            if needs_load:
                created_names = load_lora_adapter(
                    ...
                    lora_multiplier=lora.lora_multiplier,
                )
                if created_names:
        ...
        # Activate LoRA adapters for inference
        if loras:
            active_adapters = []
            adapter_weights = []
            for lora in loras:
