...
class AppInput(LLMInput, ImageCapabilityMixin):
    """Gemma 3 12B IT input model with image support."""
...
class AppOutput(LLMOutput):
    """Gemma 3 12B IT output model with token usage and timing information."""
    pass
...
class App(OpenAIChatMixin, BaseApp):
    ...
    async def run(self, input_data: AppInput) -> AsyncGenerator[Union[LLMDelta, AppOutput], None]:
        ...
        if input_data.tools or (input_data.tool_choice and input_data.tool_choice.mode in (ToolChoiceMode.REQUIRED, ToolChoiceMode.FUNCTION)):
            raise ValueError("gemma-3-12b-it does not support tool calling (tools / tool_choice)")
