...
class AppInput(BaseAppInput):
    ...
    use_teacache: bool = Field(
        default=False, description="Enable TeaCache for faster inference"
    )
...
class App(BaseApp):
    async def setup(self, metadata):
        ...
        self.infinitetalk_dir = os.path.join(
            infinitetalk_repo, "single/infinitetalk.safetensors"
        )
        ...
        self.default_frame_num = (
            125  # Default frame count for image-to-video (5 seconds)
        )
        ...
        if self.device.type == "cuda" and torch.cuda.is_available():
            gpu_vram_gb = torch.cuda.get_device_properties(self.device).total_memory / (
                1024**3
            )
        # Only offload if less than 32GB VRAM or multi-GPU setup
        self.offload_model = (
            False if gpu_vram_gb >= 32 else (True if self.world_size == 1 else False)
        )
        self.color_correction_strength = 1.0

        logging.info(
            f"GPU VRAM: {gpu_vram_gb:.1f}GB, Model offloading: {self.offload_model}"
        )
        ...
        self.wav2vec_feature_extractor, self.audio_encoder = self._custom_init(
            self.device, self.wav2vec_dir
        )
        ...
        logging.info(
            f"Enabled gradient checkpointing for: {', '.join(models_to_checkpoint)}"
        )
        ...
        class ExtraArgs:
            def __init__(
                self, use_teacache=False, teacache_thresh=0.1, size="infinitetalk-480"
            ):
    ...
    def _audio_prepare_multi(
        self, left_path, right_path, audio_type, sample_rate=16000
    ):
    ...
    def _get_embedding(self, speech_array, sr=16000, target_frames=None):
        ...
        if target_frames is not None:
            ...
            if audio_duration < required_audio_length:
                ...
                logging.info(
                    f"Audio padded from {len(speech_array) - padding_samples} to {len(speech_array)} samples ({audio_duration:.2f}s) to match target {target_frames} frames"
                )
            elif audio_duration > required_audio_length:
                # Log if audio is longer than target
                logging.info(
                    f"Audio duration ({audio_duration:.2f}s) is longer than target ({required_audio_length:.2f}s). Video will be generated for full audio length."
                )
        ...
        audio_feature = torch.from_numpy(audio_feature).to(
            dtype=torch.bfloat16, device=self.device, non_blocking=True
        )
    ...
    def _log_gpu_memory(self, prefix=""):
        ...
        logging.info(
            f"{prefix}GPU Memory: {allocated:.2f}GB allocated, {reserved:.2f}GB reserved, {free:.2f}GB free"
        )
    ...
    async def run(self, input_data: AppInput, metadata) -> AppOutput:
        ...
        video_frames = max(
            25, int(audio_duration * 25)
        )  # 25fps, minimum 25 frames (1 second)
        ...
        logging.info(
            f"Audio duration: {audio_duration:.2f}s, video frames: {video_frames} ({video_frames / 25:.2f}s video)"
        )

        audio_embedding, actual_embedding_frames = self._get_embedding(
            human_speech, target_frames=embedding_frames
        )
        ...
        save_video_ffmpeg(
            video, output_filename, [input_dict["video_audio"]], high_quality_save=False
        )
