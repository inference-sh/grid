...
class LipsyncPipeline(DiffusionPipeline):
    ...
    def loop_video(self, whisper_chunks: list, video_frames: np.ndarray, ping_pong: bool = True):
        ...
        if len(whisper_chunks) > len(video_frames):
            ...
            for i in range(num_loops):
                if (i % 2 == 1) and ping_pong:
                    # Reverse for ping-pong behavior on odd loops
                    loop_video_frames.append(video_frames[::-1])
                    loop_faces.append(faces.flip(0))
                    loop_boxes += boxes[::-1]
                    loop_affine_matrices += affine_matrices[::-1]
                else:
                    # Forward-only append (simple loop) or even loops in ping-pong mode
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print(f"LipsyncPipeline received: num_inference_steps={num_inference_steps}, guidance_scale={guidance_scale}")
        ...
        video_frames, faces, boxes, affine_matrices = self.loop_video(whisper_chunks, video_frames, ping_pong=ping_pong)
