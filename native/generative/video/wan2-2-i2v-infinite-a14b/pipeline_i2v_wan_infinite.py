...
class WanImageToVideoPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        if len(all_initial_frames) > 1:
            ...
            num_initial_images = len(all_initial_frames)
            
            if num_initial_images > num_frames:
                raise ValueError(f"Number of initial images ({num_initial_images}) cannot exceed num_frames ({num_frames})")
        else:
        ...
        if self.config.expand_timesteps:
            ...
            remaining_frames = num_frames - num_initial_images
            ...
            remaining_frames = num_frames - num_initial_images - 1
            if remaining_frames > 0:
                zero_frames = image.new_zeros(
                    image.shape[0], image.shape[1], remaining_frames, height, width
                )
        ...
        if self.config.expand_timesteps:
            ...
            # Mask out the initial frames based on number of initial images
            num_initial_latent_frames = min((num_initial_images - 1) // self.vae_scale_factor_temporal + 1, num_latent_frames)
            first_frame_mask[:, :, :num_initial_latent_frames] = 0
            return latents, latent_condition, first_frame_mask

        mask_lat_size = torch.ones(batch_size, 1, num_frames, latent_height, latent_width)

        # Create mask based on initial images and last image
        if last_image is None:
            # Mask out frames after the initial images
            if num_initial_images < num_frames:
                mask_lat_size[:, :, list(range(num_initial_images, num_frames))] = 0
        else:
            # Mask out frames between initial images and last image
            if num_initial_images < num_frames - 1:
                mask_lat_size[:, :, list(range(num_initial_images, num_frames - 1))] = 0
