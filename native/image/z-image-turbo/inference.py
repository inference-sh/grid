...
from .lora_helper import LoraConfig, manage_loras
...
# Base model filter for Civitai LoRA downloads
CIVITAI_BASE_MODEL_FILTER = ['Z-Image', 'Z-Image Turbo']
...
class AppInput(BaseAppInput):
    ...
    loras: Optional[list[LoraConfig]] = Field(
        default=None,
        description="List of LoRA configs to apply. Each config specifies adapter_name, lora_url, and lora_multiplier"
    )
...
class App(BaseApp):
    ...
    async def run(self, input_data: AppInput, metadata) -> AppOutput:
        ...
        loras = input_data.loras or []
        active_adapters = manage_loras(
            self.pipe, loras, self.loaded_loras, CIVITAI_BASE_MODEL_FILTER
        )
        ...
        if active_adapters:
            print(f"Using LoRA adapters: {active_adapters}")
