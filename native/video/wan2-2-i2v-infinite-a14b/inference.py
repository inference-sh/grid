...
class AppInput(BaseAppInput):
    video: File = Field(description="Input video for conditioning the generation")
    ...
    max_conditioning_frames: int = Field(default=8, description="Maximum number of frames to extract from input video for conditioning")
...
class App(BaseApp):
    ...
    def extract_video_frames(self, video_path: str, max_frames: int) -> List[Image.Image]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                duration = float(duration_result.stdout.strip()) if duration_result.returncode == 0 else 10.0
                
                # Calculate frame extraction rate
                fps_rate = max_frames / duration
                
                # Extract frames at calculated rate
                cmd = [
                    "ffmpeg",
                    "-i", video_path,
                    "-vf", f"fps={fps_rate}",
    ...
    async def run(self, input_data: AppInput, metadata) -> AppOutput:
        """Generate video from input video and text prompt."""
        ...
        extracted_frames = self.extract_video_frames(
            ...
            min(input_data.max_conditioning_frames, input_data.num_frames)
        )
        
        if not extracted_frames:
            raise ValueError("Could not extract any frames from the input video")
        
        # Process extracted frames
        # First frame becomes the main image
        first_frame = extracted_frames[0]
        resized_image, width, height = self.resize_image_for_pipeline(first_frame, max_area)
        
        # Remaining frames become initial_images
        initial_images = None
        if len(extracted_frames) > 1:
            initial_images = []
            for frame in extracted_frames[1:]:
                resized_frame, _, _ = self.resize_image_for_pipeline(frame, max_area)
                initial_images.append(resized_frame)
