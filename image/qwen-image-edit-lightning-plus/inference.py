...
class AppConfig:
    ...
    LORA_WEIGHT_NAME = "Qwen-Image-Lightning-8steps-V2.0-bf16.safetensors"
    DEFAULT_STEPS = 8
...
class AppInput(BaseAppInput):
    ...
    num_inference_steps: int = Field(default=AppConfig.DEFAULT_STEPS, ge=1, le=50, description="Number of denoising steps (8 steps with Lightning LoRA)")
...
class App(BaseApp):
    async def setup(self, metadata):
        """Initialize the Qwen Image Edit Plus pipeline with Lightning LoRA."""
        ...
        # Load Lightning LoRA for faster inference (8 steps)
        print(f"⚡ Loading {AppConfig.LORA_ID} LoRA...")
        self.pipeline.load_lora_weights(
            AppConfig.LORA_ID,
            weight_name=AppConfig.LORA_WEIGHT_NAME
        )
