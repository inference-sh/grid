...
MODEL_CONFIGS = {
    ...
    "dev-f16": {
        "repo": "dev", "filename": "hidream-i1-dev-BF16.gguf",
...
class AppInput(BaseAppInput):
    ...
    # Generation parameters - Dev model optimal defaults
    num_inference_steps: int = Field(
        default=28, 
        ge=1, 
        le=100, 
        description="Number of denoising steps (optimal: 28 for dev model)"
    )
    guidance_scale: float = Field(
        default=1.0,
        ge=0.0,
        le=20.0,
        description="CFG scale - how closely to follow the prompt (optimal: 1.0 for dev model)"
    )
    shift: float = Field(
        default=6.0,
        ge=0.0,
        le=10.0,
        description="Shift parameter for scheduler (optimal: 6.0 for dev model)"
    )
    scheduler_type: SchedulerType = Field(
        default=SchedulerType.FLASH_FLOW,
        description="Scheduler type (optimal: 'flash_flow' for dev model)"
    )
...
class App(BaseApp):
    async def setup(self, metadata):
        ...
        self.model = variant_mapping.get(self.variant, "dev-f16")
