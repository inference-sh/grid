...
class App(BaseApp):
    ...
    async def setup(self):
        config_file = os.path.join(
            os.path.dirname(__file__), "example/24B/24B_distill_quant_config.json"
        )
        ...
        magi_weights_path = snapshot_download(
            ...
            allow_patterns=[
                "ckpt/magi/24B_distill_quant/inference_weight.fp8.distill/*.safetensors",
                "ckpt/magi/24B_distill_quant/inference_weight.fp8.distill/*.json",
            ],
        )
        self._weights_paths["load"] = os.path.join(
            magi_weights_path, "ckpt/magi/24B_distill_quant/"
        )
