...
async def stream_completion(client, input_data, model: str) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    print(f"Stream opened header_gen={generation_id or 'unknown'} model={model}")
    ...
    try:
        async for chunk in _iter_with_timeout():
            ...
            # Log once when we first see the chunk id
            if state.get("_chunk_id") and chunks_received == 1:
                print(f"Chunk id={state['_chunk_id']} (header gen={generation_id or 'none'})")
